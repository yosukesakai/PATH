# PATH - ユーザが対象を "みる" ふるまいに応じて異なる映像を呈示するシステム  
...
写真や絵画などを見るとき、より詳細に見たいときは近づいてその部分をより大きく見たり、全体を見たいとき離れたりします。これは人々が日常的に行っていることで、 特に意識しないで実践しているふるまいです。こうした歩行やそれに準ずる移動による行動は、視覚のみならない感覚に関わる原初的なふるまい・身ぶりと考えられます。  
  
ヒトが何かを見るとき、それをどのように見たいか、どのような部分を見たいかという願望はそのふるまいに反映され、希望する“みえ”を得ることになります。対象をより 近くで見たいときは近づく、より詳しく見たいときはみつめるといった行為が、ユーザとディスプレイとの距離・位 置・滞在時間に現れているととらえ、ユーザの顔・両眼位 置を検出し呈示の仕方、呈示するコンテンツを変化させるインタラクションを 取り入れた映像・音響呈示システムの開発を試みました。  
  
位置の検出は、CV、赤外線センサなど様々な方法が考えられます。また、動きの速いものでも注視すればその動きをとらえることができる、いわば時間解像度の向上のインタラクションを、ハイスピード撮影・スロー再生を用いて導入できると考えられます。  

ユーザ位置の推定にCVによる顔検出を利用し、本システムを用いて異なる映像 (スーパースローモーションを含む) を提示する作品"PATH 01"があります。この鑑賞者は、以下のように評価しています。
...
ユーザの位置だけでなく、その他の情報をとりいれることにより、たとえば環境情報 (温度や照度など) をもとに異なるコンテンツを呈示するインテリアなどへ展開するなど、様々な応用が可能でしょう。  
